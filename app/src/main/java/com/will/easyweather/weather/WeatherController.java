...
import com.will.easyweather.bean.IndexDetail;
...
public class WeatherController {
	public static AQI convertToNewAQI(String aqiInfo, String language,
		...
		try {
			...
			int aqiValue = WeatherUtils.getAqi(aqiJSONObject
					.getString("aqi"));
			aqi.setAqi(aqiValue);
			aqi.setPm25(WeatherUtils.getAqi(aqiJSONObject.getString("pm25")));
			aqi.setPm10(WeatherUtils.getAqi(aqiJSONObject.getString("pm10")));
			aqi.setNo2(WeatherUtils.getAqi(aqiJSONObject.getString("no2")));
			aqi.setSo2(WeatherUtils.getAqi(aqiJSONObject.getString("so2")));
			aqi.setCo(WeatherConstants.NO_VALUE_FLAG.intValue());
			aqi.setO3(WeatherConstants.NO_VALUE_FLAG.intValue());
			aqi.setAqi_level(WeatherUtils.getAqiLevel(aqiValue, language));
			aqi.setAqi_desc(WeatherUtils.getAqiDesc(aqiValue, language));
			aqi.setSource(WeatherUtils.getAQISource(language));
	...
	public static Forecast convertToNewForecast(String forecastInfo,
		...
		try {
			...
			for (int i = 0; i < 6; i++)
				forecast.setType(i, WeatherUtils.getAnimationType(forecast
						.getWeatherNamesTo(i)));
			return forecast;
	...
	public static Index convertToNewIndex(String indexInfo, String language,
		...
		try {
			...
			windIndexDetail.setDesc(WeatherUtils.getWind(
					indexJSONObject.getString("fx1"),
					indexJSONObject.getString("fl1"), language));
			windIndexDetail.setTitle("风力指数");
			windIndexDetail.setDetail(WeatherUtils.getWindIndexDetail(
					indexJSONObject.getString("fl1"), language));
			windIndexDetail.setType(WeatherUtils.getIndexType("风力指数")
					.intValue());
			...
			for (Map.Entry<String, String> entry : WeatherConstants.INDEX_OLD
				...
				indexDetail.setType(WeatherUtils.getIndexType(key)
						.intValue());
				indexDetail.setTitle(WeatherUtils.getIndexTitle(key,
						language));
				String indexValue = indexJSONObject
						.getString(WeatherConstants.INDEX_OLD.get(key));
				indexDetail.setDesc(WeatherUtils.getIndexDesc(key,
						indexValue, language));

				indexDetail.setDetail(WeatherUtils.getIndexDetail(key,
						indexValue, language));
	...
	public static RealTime convertToNewRealTime(String realTimeInfo,
		...
		try {
			...
			realTime.setWeather_name(WeatherUtils.getWeatherName(
